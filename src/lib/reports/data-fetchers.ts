...
export async function fetchComplianceSummary(
  ...
  const [controls, evidences, policies, exceptions, kpis, risks] =
    await Promise.all([
      prisma.control.findMany({ where: tf, select: { status: true } }),
      prisma.evidence.findMany({ where: tf, select: { status: true } }),
      prisma.policy.findMany({ where: tf, select: { status: true } }),
      prisma.exception.findMany({ where: tf, select: { status: true } }),
      prisma.kPI.findMany({ where: tf, select: { status: true } }),
  ...
  function countByStatus<T extends { status: string }>(
    ...
    const compliant = items.filter((i) =>
      compliantStatuses.includes(i.status)
    ).length;
    const inProgress = items.filter((i) =>
      inProgressStatuses.includes(i.status)
    ).length;
  ...
  const data: Record<string, { total: number; compliant: number; inProgress: number; nonCompliant: number }> = {
    Controls: countByStatus(controls, ["Compliant"], ["Partial Compliant"]),
...
// ────────────────────────────────────────────
export async function fetchControlEffectiveness(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
  ...
  const columns: ReportColumn[] = [
    ...
    { key: "domain", header: "Domain", width: 18 },
  ...
  return {
    ...
    summary: {
      ...
      "Compliant": compliant,
      "Effectiveness Rate": pct(compliant, controls.length),
    },
  };
}
...
// ────────────────────────────────────────────
export async function fetchFrameworkCompliance(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
  ...
  const frameworks = await prisma.framework.findMany({
    ...
    include: {
      _count: { select: { controls: true, requirements: true, evidences: true } },
    },
    orderBy: { name: "asc" },
  });
...
// 5. Risk Treatment
// ────────────────────────────────────────────
export async function fetchRiskTreatment(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
...
// 6. Risk Matrix
// ────────────────────────────────────────────
export async function fetchRiskMatrix(session: Session): Promise<ReportData> {
  const tf = getTenantFilter(session);
...
// ────────────────────────────────────────────
export async function fetchEvidenceCollection(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
...
// 8. Evidence Review
// ────────────────────────────────────────────
export async function fetchEvidenceReview(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
...
export async function fetchExceptionReport(
  ...
  const rows = exceptions.map((e) => {
    let related = "";
    if (e.category === "Control" && e.control) related = e.control.name;
    else if (e.category === "Policy" && e.policy) related = e.policy.name;
    else if (e.category === "Risk" && e.risk) related = e.risk.name;
...
// 11. KPI Performance
// ────────────────────────────────────────────
export async function fetchKPIPerformance(
  session: Session
): Promise<ReportData> {
  const tf = getTenantFilter(session);
...
// Dispatcher
// ────────────────────────────────────────────
const fetchers: Record<string, (s: Session) => Promise<ReportData>> = {
  "compliance-summary": fetchComplianceSummary,
  "control-effectiveness": fetchControlEffectiveness,
  "framework-compliance": fetchFrameworkCompliance,
  "risk-assessment": fetchRiskAssessment,
  "risk-treatment": fetchRiskTreatment,
  "risk-matrix": fetchRiskMatrix,
  "evidence-collection": fetchEvidenceCollection,
  "evidence-review": fetchEvidenceReview,
  "exception-report": fetchExceptionReport,
  "policy-report": fetchPolicyReport,
  "kpi-performance": fetchKPIPerformance,
  "kpi-dashboard": fetchKPIDashboard,
};

export function getReportFetcher(
  reportType: string
