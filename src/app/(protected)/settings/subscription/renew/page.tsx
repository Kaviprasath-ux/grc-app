...
import { Home, ChevronRight, ArrowLeft, Tag, Loader2, AlertCircle } from "lucide-react";
...
export default function RenewPage() {
  ...
  async function proceedToPayment() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/settings/subscription/renew/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cycle, lines: enabledLines }),
      });
      const json = await res.json();
      if (!res.ok) {
        throw new Error(json.error || t("Checkout failed"));
      }
      toast({
      ...
      toast({ variant: "destructive", title: t("Checkout failed"), description: (e as Error).message });
    } finally {
      setSubmitting(false);
    }
  }
