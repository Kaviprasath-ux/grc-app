...
interface Risk {
  ...
  requirementRisks?: LinkedRequirement[];
}
...
export default function RiskComplianceMatrixPage() {
  ...
  const fetchRisks = useCallback(async (pageNum: number, append: boolean = false) => {
    try {
      const response = await fetch(`/api/qpost-compliance/risks?page=${pageNum}&limit=${pageSize}&includeRequirements=true`);
      if (response.ok) {
  ...
  const toggleRequirements = (riskId: string) => {
  ...
  const handleUnlinkRequirement = async (riskId: string, requirementId: string) => {
    try {
      const response = await fetch(`/api/qpost-compliance/risks/${riskId}/requirements/${requirementId}`, {
        method: "DELETE",
      });
      if (response.ok) {
        // Refresh risks to update linked requirements
        fetchRisks(1);
      }
    } catch (error) {
                ...
                <CollapsibleContent>
                  <CardContent className="pt-4">
                    ...
                    <div className="border-t pt-4">
                      <Collapsible
                        open={expandedRequirements.has(risk.id)}
                        onOpenChange={() => toggleRequirements(risk.id)}
                      >
                        <CollapsibleTrigger asChild>
                          <Button
                            variant="ghost"
                        ...
                        <CollapsibleContent className="mt-3">
                          {risk.requirementRisks && risk.requirementRisks.length > 0 ? (
                              ...
                              <TableBody>
                                {risk.requirementRisks.map((rr) => (
                                  <TableRow key={rr.id}>
                                    ...
                                    <TableCell>
                                      <Badge className={requirementStatusColors[rr.requirement.status] || "bg-gray-100"}>
                                    ...
                                    <TableCell>
                                      <Button
                                        ...
                                        onClick={() => handleUnlinkRequirement(risk.id, rr.requirement.id)}
                                      >
