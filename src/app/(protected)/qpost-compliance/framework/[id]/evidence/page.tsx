...
export default function EvidenceByFrameworkPage() {
  ...
  const { data: translatedEvidences } = useTranslatedData(allEvidences, { modelName: 'QPostEvidence' });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch evidences linked to this framework
        const evidencesResponse = await fetch(`/api/qpost-compliance/evidences?frameworkId=${frameworkId}&limit=1000`);
        if (evidencesResponse.ok) {
          const evidencesData = await evidencesResponse.json();
          setAllEvidences(evidencesData.data || []);
        }
      } catch (error) {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div style={isRTL ? { direction: 'rtl' } : undefined}>
        <h1 className="text-xl sm:text-2xl font-bold text-slate-800">
          {framework ? `${framework.name} - ${t("Evidence")}` : t("Evidence")}
        </h1>
      </div>
              ...
              <TableBody>
                {paginatedEvidences.map((evidence) => (
                  <TableRow
                    ...
                    onDoubleClick={() => router.push(`/qpost-compliance/evidence/${evidence.id}?from=framework&frameworkId=${frameworkId}&frameworkName=${encodeURIComponent(framework?.name || '')}`)}
                  >
