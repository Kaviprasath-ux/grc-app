...
const categories = ["Compliance", "Policy", "Risk"];
const statuses = [
  "Pending",
  "Approved",
  "Authorised",
...
export default function ExceptionsPage() {
  ...
  const { canView, canCreate, canEdit, canDelete, isLoading: permissionsLoading } = usePermissions('qpost-compliance.exceptions');
  ...
  const fetchExceptions = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/qpost-compliance/exceptions?${params.toString()}`);
      if (response.ok) {
  ...
  const fetchReferenceData = useCallback(async () => {
    try {
      const [usersRes, deptRes, policiesRes, risksRes, frameworksRes] =
        await Promise.all([
          fetch("/api/users"),
          fetch("/api/departments"),
          fetch("/api/qpost-compliance/policies"),
          fetch("/api/risks"),
  ...
  const fetchRequirements = useCallback(async (frameworkId: string) => {
    ...
    try {
      const response = await fetch(`/api/qpost-compliance/requirements?frameworkId=${frameworkId}`);
      if (response.ok) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (savedId) {
          triggerTranslation('QPostException', savedId, {
            name: createForm.name,
            description: createForm.description,
          });
        }
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/qpost-compliance/exceptions/${selectedException.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editForm.name,
      ...
      if (response.ok) {
        triggerTranslation('QPostException', selectedException.id, {
          name: editForm.name,
          description: editForm.description,
        });
        toast({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex ltr:sm:justify-end rtl:sm:justify-start">
        <PermissionGate resource="qpost-compliance.exceptions" action="create">
                    ...
                    <TableCell className="py-3 pr-5 text-sm">
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => router.push(`/qpost-compliance/exceptions/${exception.id}`)}
                          title={t("View")}
                          className="h-7 w-7 text-slate-400 hover:text-primary-600 hover:bg-primary-50"
                        >
                          <Eye className="h-3.5 w-3.5" />
                        </Button>
                        <PermissionGate resource="qpost-compliance.exceptions" action="edit">
                          <Button
                            variant="ghost"
                            size="icon"
                        ...
                        <PermissionGate resource="qpost-compliance.exceptions" action="delete">
                          <Button
                            variant="ghost"
                            size="icon"
