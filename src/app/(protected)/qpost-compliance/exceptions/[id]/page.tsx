...
export default function ExceptionDetailPage({
  ...
  const { data: translatedException } = useTranslatedRecord(exception, { modelName: 'QPostException' });
  ...
  const fetchException = useCallback(async () => {
    try {
      const response = await fetch(`/api/qpost-compliance/exceptions/${id}`);
      if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        triggerTranslation('QPostException', id, {
          name: formData.name,
          description: formData.description,
        });
        fetchException();
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch(`/api/qpost-compliance/exceptions/${id}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          content: newComment,
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        router.push("/qpost-compliance/exceptions");
      }
    } catch (error) {
  ...
  const handleSendBack = async () => {
    ...
    try {
      ...
      const commentResponse = await fetch(`/api/qpost-compliance/exceptions/${id}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <nav className="flex items-center gap-1.5 text-sm overflow-x-auto whitespace-nowrap">
          ...
          <Link href="/qpost-compliance/exceptions" className="text-slate-500 hover:text-primary-600 transition-colors">
            {t("Exceptions")}
          </Link>
          <ChevronRight className="h-3.5 w-3.5 text-slate-300 ltr:rotate-0 rtl:rotate-180" />
