...
export default function KPIDetailPage({
  ...
  const { data: translatedKpi } = useTranslatedRecord(kpi, { modelName: 'QPostKPI' });
  ...
  const fetchKPI = useCallback(async () => {
    try {
      const response = await fetch(`/api/qpost-compliance/kpis/${id}`);
      if (response.ok) {
  ...
  const handleAddActualScore = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/qpost-compliance/kpis/${id}/reviews`, {
        method: "POST",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok && kpi?.evidence?.id) {
        await fetch(`/api/qpost-compliance/evidences/${kpi.evidence.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        triggerTranslation('QPostKPI', id, {
          objective: formData.objective,
          description: formData.description,
          dataSource: formData.dataSource,
          calculationFormula: formData.calculationFormula,
  ...
  const handleSaveInlineActualScore = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetch(`/api/qpost-compliance/kpis/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            actualScore: actualScoreValue,
  ...
  const handleUpdateReview = async () => {
    try {
      ...
      const response = await fetch(
        `/api/qpost-compliance/kpis/${id}/reviews/${updateForm.reviewId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleDeleteReview = async () => {
    ...
    try {
      const response = await fetch(`/api/qpost-compliance/kpis/${id}/reviews/${reviewToDelete}`, {
        method: "DELETE",
      });
  ...
  const handleCreateActionPlan = async () => {
    ...
    try {
      const response = await fetch(
        `/api/qpost-compliance/kpis/${id}/reviews/${actionPlanReviewId}/action-plans`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <nav className="flex items-center gap-1.5 text-sm overflow-x-auto whitespace-nowrap">
        ...
        <Link href="/qpost-compliance/kpis" className="text-slate-500 hover:text-primary-600 transition-colors">
          {t("KPIs")}
        </Link>
        <ChevronRight className="h-3.5 w-3.5 text-slate-300 ltr:rotate-0 rtl:rotate-180" />
