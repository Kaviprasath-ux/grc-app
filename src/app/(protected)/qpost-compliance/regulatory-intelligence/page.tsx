...
export default function RegulatoryIntelligenceHubPage() {
  ...
  const fetchProfiles = useCallback(async () => {
    try {
      const res = await fetch("/api/qpost-compliance/regulatory-intelligence/profiles");
      if (res.ok) {
  ...
  const fetchRegulations = useCallback(async () => {
    try {
      const res = await fetch("/api/qpost-compliance/regulatory-intelligence/regulations");
      if (res.ok) {
        const json = await res.json();
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/qpost-compliance/regulatory-intelligence/profiles/${deleteId}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleSuggestFrameworks = async (profileId: string) => {
    ...
    try {
      const res = await fetch("/api/qpost-compliance/regulatory-intelligence/suggest", {
        method: "POST",
  ...
  const handleOnboardFramework = () => {
    ...
    router.push(`/qpost-compliance/framework?${params.toString()}`);
  };
      ...
      <Tabs defaultValue="organisation-profile" className="w-full">
        ...
        <TabsContent value="organisation-profile" className="mt-4">
          ...
          <div className="flex justify-end mb-4">
            <Button onClick={() => router.push("/qpost-compliance/regulatory-intelligence/add-profile")}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => router.push(`/qpost-compliance/regulatory-intelligence/edit-profile/${profile.id}`)}
                          >
