...
export default function GovernanceMasterDataPage() {
  ...
  useEffect(() => {
    ...
    fetchDepartments();
    fetchUsers();
  }, [fetchPolicies, fetchDepartments, fetchUsers]);
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await fetch(`/api/qpost-compliance/policies/${selectedPolicy.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          code: formData.code,
  ...
  const confirmDeleteAll = async () => {
    try {
      const response = await fetch("/api/qpost-compliance/policies/delete-all", {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <nav className="flex items-center gap-1.5 text-sm overflow-x-auto whitespace-nowrap">
        ...
        <ChevronRight className="h-3.5 w-3.5 text-slate-300 ltr:rotate-0 rtl:rotate-180" />
        <Link href="/qpost-compliance/master-data" className="text-slate-500 hover:text-primary-600 transition-colors">
        ...
        <Table className="min-w-[600px]">
          <TableHeader>
            <TableRow className="border-b border-slate-100 bg-slate-50 hover:bg-slate-50">
              ...
              <TableHead className="text-xs font-medium text-slate-500 uppercase tracking-wider py-3">{t("Control")}</TableHead>
            ...
            {[1, 2].map((step) => (
              <div key={step} className="flex items-center">
                ...
                <span className={`hidden sm:inline ltr:ml-2 rtl:mr-2 text-sm ${
                  ...
                  {step === 1 ? t("Policy Information") : t("Review")}
                </span>
