...
export default function FrameworkMasterDataPage() {
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await fetch(`/api/qpost-compliance/frameworks/${selectedFramework.id}`, {
        method: "PUT",
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (selectedFile) {
          ...
          formDataUpload.append("useAI", String(useAIRequirements));

          await fetch("/api/qpost-compliance/frameworks/import", {
            method: "POST",
            body: formDataUpload,
          });
        }
