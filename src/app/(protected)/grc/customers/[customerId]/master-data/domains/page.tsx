...
import { Pencil, Trash2, Home, ChevronRight, Globe } from "lucide-react";
...
export default function CustomerMasterDataDomainsPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`/api/grc/customers/${customerId}/domains`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/control-domains/${editItem.id}`, {
        method: "PUT",
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/control-domains/${deleteItem.id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const breadcrumb = (
    <nav className="flex items-center gap-1.5 text-sm overflow-x-auto whitespace-nowrap">
      ...
      <ChevronRight className="h-3.5 w-3.5 text-slate-300 ltr:rotate-0 rtl:rotate-180" />
      <span className="text-primary-700 font-medium">{t("Domains")}</span>
    </nav>
  );
  ...
  return (
    <div className="space-y-6">
      {breadcrumb}
      <h1 className="text-xl sm:text-2xl font-bold text-slate-800">{t("Domains")}</h1>

      <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto">
        <Table className="min-w-[400px]">
          <TableHeader>
            <TableRow className="border-b border-slate-100 bg-slate-50 hover:bg-slate-50">
              <TableHead className="text-xs font-medium text-slate-500 uppercase tracking-wider h-auto py-3 ps-5">{t("Domain Name")}</TableHead>
              <TableHead className="text-xs font-medium text-slate-500 uppercase tracking-wider h-auto py-3">{t("Controls Count")}</TableHead>
              ...
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={3}>
                  <div className="py-16 text-center">
                    <div className="w-12 h-12 rounded-lg bg-primary-50 flex items-center justify-center mx-auto mb-3"><Globe className="h-6 w-6 text-primary-500" /></div>
                    <h3 className="text-sm font-medium text-slate-600 mb-1">{t("No Domains Found")}</h3>
                    <p className="text-xs text-slate-400">{t("No domains found for this customer.")}</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : items.map((domain) => (
              <TableRow key={domain.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                <TableCell className="py-3.5 ps-5 text-sm text-slate-800">{domain.name}</TableCell>
                <TableCell className="py-3.5 text-sm text-slate-600">{domain._count.controls}</TableCell>
                <TableCell className="py-3.5 pe-5">
                  <div className="flex items-center gap-0.5">
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-primary-600 hover:bg-primary-50" onClick={() => handleEdit(domain.id, domain.name)} title={t("Edit")}><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-semantic-error hover:bg-red-50" onClick={() => handleDelete(domain.id, domain.name)} title={t("Delete")}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-[700px] p-0 gap-0 overflow-hidden" onOpenAutoFocus={(e) => e.preventDefault()}>
          <div className="px-6 py-5 border-b border-slate-100">
            <DialogHeader><DialogTitle className="text-lg font-semibold text-slate-800">{t("Edit Domain")}</DialogTitle></DialogHeader>
          </div>
