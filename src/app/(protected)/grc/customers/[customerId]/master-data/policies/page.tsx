...
import { Pencil, Trash2, Home, ChevronRight, FileText } from "lucide-react";
...
export default function CustomerMasterDataPoliciesPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`/api/grc/customers/${customerId}/policies`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/policies/${editItem.id}`, {
        method: "PUT",
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/policies/${deleteItem.id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      {breadcrumb}
      <h1 className="text-xl sm:text-2xl font-bold text-slate-800">{t("Policies")}</h1>
        ...
        <Table className="min-w-[600px]">
          <TableHeader>
            <TableRow className="border-b border-slate-100 bg-slate-50 hover:bg-slate-50">
              ...
              <TableHead className="text-xs font-medium text-slate-500 uppercase tracking-wider h-auto py-3">{t("Version")}</TableHead>
              ...
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={7}>
                  <div className="py-16 text-center">
                    <div className="w-12 h-12 rounded-lg bg-primary-50 flex items-center justify-center mx-auto mb-3"><FileText className="h-6 w-6 text-primary-500" /></div>
                    <h3 className="text-sm font-medium text-slate-600 mb-1">{t("No Policies Found")}</h3>
                    <p className="text-xs text-slate-400">{t("No policies found for this customer.")}</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : items.map((policy) => (
              <TableRow key={policy.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
                <TableCell className="py-3.5 ps-5 text-sm text-slate-800">{policy.code}</TableCell>
                <TableCell className="py-3.5 text-sm text-slate-600">{policy.name}</TableCell>
                <TableCell className="py-3.5 text-sm text-slate-600">{policy.documentType || "-"}</TableCell>
                <TableCell className="py-3.5 text-sm text-slate-600">{policy.version || "-"}</TableCell>
                <TableCell className="py-3.5 text-sm text-slate-600">{policy.department?.name || "-"}</TableCell>
                <TableCell className="py-3.5"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${policy.status === "Approved" ? "bg-green-50 text-green-700 border-green-200" : policy.status === "Draft" ? "bg-amber-50 text-amber-700 border-amber-200" : policy.status === "In Review" ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-slate-50 text-slate-700 border-slate-200"}`}>{policy.status}</span></TableCell>
                <TableCell className="py-3.5 pe-5">
                  <div className="flex items-center gap-0.5">
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-primary-600 hover:bg-primary-50" onClick={() => handleEdit(policy.id, policy.name)} title={t("Edit")}><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-semantic-error hover:bg-red-50" onClick={() => handleDelete(policy.id, policy.name)} title={t("Delete")}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-[700px] p-0 gap-0 overflow-hidden" onOpenAutoFocus={(e) => e.preventDefault()}>
          <div className="px-6 py-5 border-b border-slate-100">
            <DialogHeader><DialogTitle className="text-lg font-semibold text-slate-800">{t("Edit Policy")}</DialogTitle></DialogHeader>
          </div>
