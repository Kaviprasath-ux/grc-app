...
// GET /api/qpost-compliance/artifacts - List all QPost artifacts
export const GET = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      const tenantFilter = getTenantFilter(session);

      const artifacts = await prisma.qPostArtifact.findMany({
        where: tenantFilter,
        include: {
          uploader: {
      ...
      console.error("Error fetching QPost artifacts:", error);
      return NextResponse.json(
  ...
  { resource: "qpost-compliance.artifacts", action: "view" }
);

// POST /api/qpost-compliance/artifacts - Upload new QPost artifact
export const POST = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const count = await prisma.qPostArtifact.count({
      ...
      console.error("Error uploading QPost artifact:", error);
      return NextResponse.json(
  ...
  { resource: "qpost-compliance.artifacts", action: "create" }
);
