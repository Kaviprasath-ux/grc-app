...
// POST import QPost evidences from CSV
export const POST = withAuth(
  ...
  try {
    ...
    const existingEvidences = await prisma.qPostEvidence.findMany({
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        await prisma.qPostEvidence.create({
          data: {
            customerAccountId,
    ...
    console.error("Error importing QPost evidences:", error);
    return NextResponse.json(
