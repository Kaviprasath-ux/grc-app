...
export async function GET(
  ...
  try {
    ...
    console.error("Error fetching QPost evidence:", error);
    return NextResponse.json(
...
export const PUT = withAuth(
  async (req, context, session) => {
    try {
      ...
      const updatedEvidence = await prisma.qPostEvidence.update({
        where: { id },
        data: updateData,
        include: {
          framework: true,
        ...
        for (const manager of managers) {
          if (manager.id !== session.id) {
            await notificationService.send({
              ...
              link: `/qpost-compliance/evidence/${updatedEvidence.id}`,
              metadata: {
      ...
      console.error("Error updating QPost evidence:", error);
...
export async function DELETE(
  ...
  try {
    ...
    const existing = await prisma.qPostEvidence.findUnique({
      where: { id },
    ...
    console.log(`[QPost Evidence Delete] Deleting evidence: ${existing.evidenceCode} (${id})`);

    // Step 1: Clean up AI documents from RunPod
    try {
      const aiResults = await aiDeleteService.deleteAllForEvidence(id);
      console.log(
        `[QPost Evidence Delete] AI cleanup: ${aiResults.filter((r) => r.status === "deleted").length}/${aiResults.length} documents deleted`
      );
    } catch (aiError) {
      console.warn("[QPost Evidence Delete] AI cleanup failed (continuing):", aiError);
    ...
    for (const attachment of existing.attachments) {
      if (attachment.filePath) {
        try {
          ...
          console.log(`[QPost Evidence Delete] Deleted file: ${attachment.fileName}`);
        } catch (fileError) {
          console.warn(
            `[QPost Evidence Delete] Could not delete file: ${attachment.filePath}`
          );
        }
      }
    }
    ...
    await prisma.qPostEvidence.delete({
      where: { id },
    });

    console.log(`[QPost Evidence Delete] Successfully deleted evidence: ${existing.evidenceCode}`);
    return NextResponse.json({ message: "Evidence deleted successfully" });
  } catch (error: unknown) {
    console.error("Error deleting QPost evidence:", error);
    if ((error as { code?: string }).code === "P2025") {
      return NextResponse.json(
        { error: "Evidence not found" },
        { status: 404 }
