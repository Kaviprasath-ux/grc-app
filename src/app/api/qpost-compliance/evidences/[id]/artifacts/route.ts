...
// GET all artifacts linked to QPost evidence - with tenant validation
export const GET = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      const evidenceArtifacts = await prisma.qPostEvidenceArtifact.findMany({
        where: { evidenceId: id },
        include: {
          artifact: {
            include: {
      ...
      console.error("Error fetching QPost evidence artifacts:", error);
      return NextResponse.json(
...
// POST link artifact to QPost evidence - with tenant validation
export const POST = withAuth(
  async (req, context: RouteContext, session) => {
    ...
    for (const artId of idsToLink) {
      ...
      const existingLink = await prisma.qPostEvidenceArtifact.findFirst({
        where: {
          evidenceId: id,
          artifactId: artId,
        },
      ...
      // Create the link
      const evidenceArtifact = await prisma.qPostEvidenceArtifact.create({
        data: {
          evidenceId: id,
          artifactId: artId,
        },
      ...
      console.error("Error linking artifact to QPost evidence:", error);
      return NextResponse.json(
...
// DELETE unlink artifact from QPost evidence - with tenant validation
  ...
  async (req, context: RouteContext, session) => {
    try {
      ...
      if (artifactId) {
        ...
        const link = await prisma.qPostEvidenceArtifact.findFirst({
          where: {
            evidenceId: id,
            artifactId,
          },
        ...
        await prisma.qPostEvidenceArtifact.delete({
          where: { id: link.id },
        });
      ...
      console.error("Error unlinking artifact from QPost evidence:", error);
      return NextResponse.json(
