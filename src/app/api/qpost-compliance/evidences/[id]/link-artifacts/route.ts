...
// POST - Link artifacts to QPost evidence
  ...
  async (req, context: RouteContext) => {
    try {
      ...
      for (const artifactId of artifactIds) {
        try {
          const link = await prisma.qPostEvidenceArtifact.create({
            data: {
              evidenceId: id,
              artifactId,
            },
          ...
          console.log(`QPost Artifact ${artifactId} already linked or error:`, err);
        }
      }
      ...
      console.error("Error linking QPost artifacts:", error);
      return NextResponse.json(
...
// DELETE - Unlink artifact from QPost evidence
export const DELETE = withAuth(
  async (req, context: RouteContext) => {
    try {
      ...
      console.error("Error unlinking QPost artifact:", error);
      return NextResponse.json(
