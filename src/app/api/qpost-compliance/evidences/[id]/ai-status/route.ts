...
/**
 * GET /api/qpost-compliance/evidences/[id]/ai-status
 * Get complete AI status for a QPost evidence
  ...
  async (req: NextRequest, context: RouteContext, session) => {
    try {
      ...
      if (latestIngestJob && !["completed", "failed", "ingested"].includes(latestIngestJob.status.toLowerCase())) {
        ...
        console.log(`[QPost AI] GET  ${AI_CONFIG.endpoints.ingestStatus}/${jobId} → polling from ai-status`);
        ...
        try {
          ...
          if (runpodResponse.ok) {
            ...
            console.log(`[QPost AI] GET  ${AI_CONFIG.endpoints.ingestStatus}/${jobId} → ${newStatus}`, JSON.stringify(runpodData));

            // Update job and evidence if status changed
            if (newStatus !== latestIngestJob.status.toLowerCase()) {
              await prisma.qPostEvidenceAIIngestJob.update({
              ...
              if (newStatus === "completed") {
                console.log(`[QPost AI] Ingest completed, calling ingest-result to fetch result and trigger review`);
                try {
                  ...
                  if (ingestResultResponse.ok) {
                    ...
                    console.log(`[QPost AI] ingest-result response:`, JSON.stringify(resultData));

                    // If review was auto-triggered, update local evidence data
                    if (resultData.reviewTriggered) {
                      console.log(`[QPost AI] Review auto-triggered successfully`);
                    }
                  } else {
                    console.error(`[QPost AI] Failed to call ingest-result: ${ingestResultResponse.status}`);
                  }
                } catch (resultError) {
                  console.error(`[QPost AI] Error calling ingest-result:`, resultError);
                }
              }
            }
          } else {
            console.log(`[QPost AI] GET  ${AI_CONFIG.endpoints.ingestStatus}/${jobId} → ${runpodResponse.status} (error)`);
          }
        } catch (pollError) {
          console.log(`[QPost AI] GET  ${AI_CONFIG.endpoints.ingestStatus}/${jobId} → poll error:`, pollError);
        }
      }

      // Refetch evidence to get latest review data (in case auto-trigger completed)
      const updatedEvidence = await prisma.qPostEvidence.findUnique({
        where: { id },
        select: {
          aiReviewStatus: true,
          aiReviewedAt: true,
      ...
      console.log(`[QPost AI] GET /api/qpost-compliance/evidences/${id}/ai-status → ingest: ${ingestStatus}, review: ${normalizedReviewStatus}`);
      ...
      console.error("Error fetching QPost AI status:", error);
      return NextResponse.json(
