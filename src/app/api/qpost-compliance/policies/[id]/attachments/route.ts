...
// GET /api/qpost-compliance/policies/[id]/attachments - Get attachments for a policy
export const GET = withAuth(
  async (req: NextRequest, context: RouteContext, session) => {
    try {
      ...
      const attachments = await prisma.qPostPolicyAttachment.findMany({
        where: { policyId: id },
...
// POST /api/qpost-compliance/policies/[id]/attachments - Upload attachment for a policy
export const POST = withAuth(
  async (req: NextRequest, context: RouteContext, session) => {
    try {
      ...
      const uploadDir = join(process.cwd(), "uploads", "qpost-governance", id);
      await mkdir(uploadDir, { recursive: true });
      await writeFile(join(uploadDir, uniqueFileName), buffer);
      const relativePath = `/uploads/qpost-governance/${id}/${uniqueFileName}`;

      // Create attachment record
      const attachment = await prisma.qPostPolicyAttachment.create({
        data: {
          policyId: id,
      ...
      if (policy.status === "Not Uploaded") {
        await prisma.qPostPolicy.update({
          where: { id },
          data: { status: "Draft" },
        });
      }
...
// DELETE /api/qpost-compliance/policies/[id]/attachments?attachmentId=xxx - Delete an attachment
export const DELETE = withAuth(
  async (req: NextRequest, context: RouteContext, session) => {
    try {
      ...
      const attachment = await prisma.qPostPolicyAttachment.findUnique({
      ...
      await prisma.qPostPolicyAttachment.delete({
        where: { id: attachmentId },
      });

      // Also delete matching vault document (bidirectional sync)
      try {
        const matchingVaultDoc = await prisma.qPostGovernanceVaultDocument.findFirst({
          where: {
