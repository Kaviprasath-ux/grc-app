...
export const GET = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      const policy = await prisma.qPostPolicy.findFirst({
...
export const PUT = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      const existing = await prisma.qPostPolicy.findUnique({
        where: { id },
      ...
      const policy = await prisma.qPostPolicy.update({
        where: { id },
        data: {
          name,
          version,
...
export const DELETE = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      console.log(`[QPost Policy Delete] Deleting policy: ${existing.code} (${id})`);
      ...
      try {
        ...
        console.log(
          `[QPost Policy Delete] AI cleanup: ${aiResults.filter((r) => r.status === "deleted").length}/${aiResults.length} documents deleted`
        );
      } catch (aiError) {
        console.warn("[QPost Policy Delete] AI cleanup failed (continuing):", aiError);
      ...
      for (const attachment of existing.attachments) {
        if (attachment.filePath) {
          try {
            ...
            console.log(`[QPost Policy Delete] Deleted file: ${attachment.fileName}`);
          } catch (fileError) {
            console.warn(
              `[QPost Policy Delete] Could not delete file: ${attachment.filePath}`
            );
          }
        }
      }
      ...
      await prisma.qPostPolicy.delete({
        where: { id },
      });

      console.log(`[QPost Policy Delete] Successfully deleted policy: ${existing.code}`);
