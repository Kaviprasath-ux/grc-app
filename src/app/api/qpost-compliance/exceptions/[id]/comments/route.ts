...
// GET all comments for a QPost exception
export const GET = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      const comments = await prisma.qPostExceptionComment.findMany({
      ...
      console.error("Error fetching QPost exception comments:", error);
      return NextResponse.json(
...
// POST create new comment on a QPost exception
export const POST = withAuth(
  async (req, context: RouteContext, session) => {
    try {
      ...
      const comment = await prisma.qPostExceptionComment.create({
        data: {
          exceptionId: id,
          content,
          userId: session.id,
      ...
      if (isApproverCommenting && exception.requesterId && exception.requesterId !== session.id && exception.customerAccountId) {
        await notificationService.send({
          ...
          link: `/qpost-compliance/exceptions/${exception.id}`,
      ...
      console.error("Error creating QPost exception comment:", error);
      return NextResponse.json(
