  ...
  async (req, context, session) => {
    try {
      ...
      const [exceptions, total] = await Promise.all([
        ...
        prisma.qPostException.count({ where }),
      ]);
      ...
      console.error("Error fetching QPost exceptions:", error);
      return NextResponse.json(
...
// POST create new QPost exception - with customer account assignment
  ...
  async (req, context, session) => {
    try {
      ...
      if (!code) {
        ...
        const lastException = await prisma.qPostException.findFirst({
      ...
      const exception = await prisma.qPostException.create({
        data: {
          customerAccountId,
          exceptionCode: code,
          name,
      ...
      console.error("Error creating QPost exception:", error);
