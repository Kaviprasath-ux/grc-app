...
/**
 * GET /api/tprm/assessment-factory/status/[jobId]
 * Proxies to RunPod GET /api/factory_ingest_status/{job_id}
 */
export const GET = withAuth(
  async (_req: NextRequest, context: RouteContext) => {
    try {
      ...
      const url = getExternalApiUrl('PYTHON_BACKEND', `/api/factory_ingest_status/${encodeURIComponent(jobId)}`);
      const res = await fetch(url, { method: 'GET', headers: { auth: secret } });
      const resText = await res.text();

      if (!res.ok) {
        let errBody: Record<string, unknown> = { error: 'Status check failed' };
      ...
      console.error('[Assessment Factory Status] Error:', error);
