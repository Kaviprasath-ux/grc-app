  ...
  try {
    ...
    return new NextResponse(csvContent, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=Vulnerability-Import-Template.csv",
      },
    });
  } catch (error) {
...
// POST - Import vulnerabilities from uploaded data
  ...
  async (request: NextRequest, context, session) => {
    try {
      ...
      if (!customerAccountId) {
        return NextResponse.json(
          { error: "Customer account not found. Cannot import vulnerabilities." },
          { status: 400 }
        );
      }
      ...
      const [existingVulnerabilities, existingCategories] = await Promise.all([
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          // Check for duplicate
          const existingVuln = vulnerabilitiesCache.find(
          ...
          if (categoryName) {
            ...
            if (!category) {
              // Create new category
              category = await prisma.vulnerabilityCategory.create({
                data: {
                  customerAccountId,
                  name: categoryName,
                },
          ...
          results.success++;
          results.created.push({ name: vulnerability.name });
        } catch (error) {
      ...
      if (results.success === 0 && results.failed > 0) {
        return NextResponse.json(
          {
            error: "Import failed. No vulnerabilities were created.",
            results,
          },
          { status: 400 }
        );
      ...
      let message = `Successfully imported ${results.success} vulnerability(ies)`;
      ...
      return NextResponse.json(
        { error: "Failed to import vulnerabilities. Please try again." },
        { status: 500 }
      );
    }
  },
