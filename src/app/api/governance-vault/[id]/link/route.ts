...
// POST link governance (policies) to vault document
  ...
  async (req, context: RouteContext, session) => {
    try {
      ...
      if (governanceIds.length > 0) {
        await prisma.governanceVaultDocumentLink.createMany({
      ...
      console.error("Error linking governance to document:", error);
      return NextResponse.json(
