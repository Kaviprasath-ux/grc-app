...
export async function POST(
  ...
  try {
    ...
    const existingLink = await prisma.policyException.findFirst({
      where: {
        policyId: id,
        exceptionId,
      },
    ...
    // Create the link
    const policyException = await prisma.policyException.create({
      data: {
        policyId: id,
        exceptionId,
      },
...
export async function GET(
  ...
  try {
    ...
    const policyExceptions = await prisma.policyException.findMany({
      where: { policyId: id },
      include: {
        exception: true,
      },
