...
// GET all locations - filtered by audit head
export const GET = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const locations = await prisma.auditLocation.findMany({
...
// POST create a new location - assigned to current audit head
export const POST = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const existing = await prisma.auditLocation.findFirst({
        where: {
          name,
          auditHeadId,
        },
      ...
      const location = await prisma.auditLocation.create({
        data: {
          name,
          customerAccountId,
          auditHeadId,
