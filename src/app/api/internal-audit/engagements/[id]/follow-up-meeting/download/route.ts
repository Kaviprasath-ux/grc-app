...
// GET - Download the Follow-up Meeting Form as a print-friendly PDF.
export const GET = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const form = await prisma.auditFollowUpMeeting.findUnique({ where: { engagementId: id } });

      const attendees: Array<Record<string, string>> = form?.attendees
        ? JSON.parse(form.attendees)
        : [];
      let recommendations: Array<Record<string, string>> = form?.recommendations
      ...
      if (recommendations.length === 0) {
        const findings = await prisma.internalAuditFinding.findMany({
          ...
          select: { recommendation: true, responsiblePerson: true, targetDate: true, status: true },
      ...
      // Generic bordered table. cols: { header, key, width(0..1 of contentW) }
      const drawTable = (
        ...
        const drawRow = (cells: string[], f: PDFFont, isHeader: boolean) => {
          // Compute row height from wrapped lines per cell.
      ...
      // ----- Document -----
      drawCentered("Follow-up Meeting Form for the Implementation of the Recommendations", 14, bold);
      drawCentered(
      ...
      // Meeting header table (label/value as a 2-col grid using 3 pairs per row → render as key/value list)
      ...
      sectionTitle("Status of Implementation of Recommendations");
      drawTable(
        [
          { header: "#", key: "number", w: 0.05 },
          { header: "Recommendation", key: "recommendation", w: 0.3 },
          { header: "Official", key: "official", w: 0.14 },
          { header: "Due Date", key: "implementationDate", w: 0.12 },
          { header: "Status", key: "implementationStatus", w: 0.14 },
          { header: "Progress %", key: "progress", w: 0.09 },
          { header: "Notes", key: "notes", w: 0.16 },
        ],
        recommendations,
        3
      );
      ...
      const safeName = `Follow-up-Meeting-${engagement.auditId || id}.pdf`;
      ...
      console.error("Error generating follow-up meeting PDF:", error);
