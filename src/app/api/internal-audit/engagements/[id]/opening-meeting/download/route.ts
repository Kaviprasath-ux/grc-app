...
// GET - Download the Audit Task Opening Meeting Minutes as a print-friendly PDF.
export const GET = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const form = await prisma.auditOpeningMeeting.findUnique({ where: { engagementId: id } });

      const attendees: Array<Record<string, string>> = form?.attendees
        ? JSON.parse(form.attendees)
        : [];
      const topics: Array<Record<string, string>> = form?.topicsDiscussed
        ? JSON.parse(form.topicsDiscussed)
        : [];
      const actions: Array<Record<string, string>> = form?.agreedActions
      ...
      const paragraph = (text: string) => {
        for (const ln of wrap(text, 9.5, font, contentW)) {
          ensureSpace(13);
          page.drawText(ln, { x: margin, y: y - 10, size: 9.5, font, color: rgb(0.15, 0.15, 0.15) });
          y -= 13;
        }
      };
      ...
      // ----- Document -----
      drawCentered("Audit Task Opening Meeting Minutes", 14, bold);
      drawCentered(
      ...
      sectionTitle("Objective of the meeting");
      paragraph(form?.objective || "");
      ...
      drawTable(
        [
          ...
          { header: "Subject", key: "subject", w: 0.34 },
          { header: "Details / Notes", key: "details", w: 0.6 },
        ],
        topics,
        5
      );
      ...
      drawTable(
        [
          { header: "#", key: "number", w: 0.06 },
          { header: "Action required", key: "actionRequired", w: 0.48 },
          { header: "Official", key: "official", w: 0.24 },
          { header: "Implementation Date", key: "implementationDate", w: 0.22 },
        ],
        actions,
        3
      );
      ...
      const safeName = `Opening-Meeting-${engagement.auditId || id}.pdf`;
      ...
      console.error("Error generating opening meeting PDF:", error);
