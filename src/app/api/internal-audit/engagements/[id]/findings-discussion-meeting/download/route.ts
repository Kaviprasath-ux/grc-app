...
// GET - Download the Preliminary Observations Discussion Meeting form as PDF.
export const GET = withAuth(
  async (req: NextRequest, context, session) => {
    try {
      ...
      const form = await prisma.auditFindingsDiscussionMeeting.findUnique({
        where: { engagementId: id },
      });

      const attendees: Array<Record<string, string>> = form?.attendees
        ? JSON.parse(form.attendees)
        : [];
      let notes: Array<Record<string, string>> = form?.notesDiscussed
        ? JSON.parse(form.notesDiscussed)
        : [];
      let agreed: Array<Record<string, string>> = form?.agreedActions
        ? JSON.parse(form.agreedActions)
        : [];

      // Fall back to findings-derived rows if nothing saved yet.
      if (notes.length === 0 || agreed.length === 0) {
      ...
      const sectionTitle = (text: string) => {
        y -= 12;
        ensureSpace(16);
        page.drawText(text, { x: margin, y: y - 12, size: 11, font: bold, color: rgb(0, 0, 0) });
        y -= 18;
      };
      ...
      const drawTable = (
        ...
        const dataRows =
          rows.length >= minRows ? rows : [...rows, ...Array(minRows - rows.length).fill({})];
      ...
      // ----- Document -----
      drawCentered("Preliminary Observations Discussion Meeting", 14, bold);
      drawCentered(
      ...
      // Header table (single row of meeting metadata)
      drawTable(
        [
          { header: "Meeting Venue", key: "meetingVenue", w: 0.17 },
          { header: "History", key: "history", w: 0.15 },
          { header: "Assignment Title", key: "assignmentTitle", w: 0.18 },
          { header: "Audit Task Number", key: "auditTaskNumber", w: 0.18 },
          { header: "Department", key: "department", w: 0.16 },
          { header: "Management", key: "management", w: 0.16 },
        ],
        [
          {
            meetingVenue: form?.meetingVenue || "",
            history: form?.history || "",
            assignmentTitle: form?.assignmentTitle || engagement.engagementTitle || "",
            auditTaskNumber: form?.auditTaskNumber || engagement.auditId || "",
      ...
      drawTable(
        [
          ...
          { header: "Note", key: "note", w: 0.3 },
          { header: "Degree of Risk", key: "degreeOfRisk", w: 0.13 },
          { header: "Management Response", key: "managementResponse", w: 0.26 },
          { header: "Proposed Action", key: "proposedAction", w: 0.26 },
        ],
        notes,
        2
      );
      ...
      drawTable(
        [
          ...
          { header: "Procedure", key: "procedure", w: 0.5 },
          { header: "Official", key: "official", w: 0.24 },
          { header: "Implementation Date", key: "implementationDate", w: 0.2 },
        ],
        agreed,
        3
      );
